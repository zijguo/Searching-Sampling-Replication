...
### Author: Zhenyu Wang
### Date: 05/22/2022
### Aim: Draw histograms of point estimators for TSHT and CIIV for S2 when homo errors.
...
for(i.n in 1:length(n.options)){
  for(i.VIO.str in 1:length(VIO.str.options)){
    ...
    ########### Part-2: Simulations ##########
    # matrix storing info
    Point.mat = matrix(NA, nrow=nsim, ncol=2)
    colnames(Point.mat) = c("TSHT", "CIIV")
    ...
    for(i.sim in 1:nsim){
      ...
      Point.mat[i.sim, 1] = TSHT.temp$betaHat
      CI = TSHT.temp$ci
      CI.mat[i.sim, 1:2] = CI
      ...
      Point.mat[i.sim, 2] = CIIV.result$Coefficients_CIM[1]
    ...
    Point.mat.list[[(i.n-1)*length(VIO.str.options)+i.VIO.str]] = Point.mat
    CI.mat.list[[(i.n-1)*length(VIO.str.options)+i.VIO.str]] = CI.mat
  }
}
...
for(i.n in 1:length(n.options)){
  for(i.VIO.str in 1:length(VIO.str.options)){
    n = n.options[i.n]
    VIO.str = VIO.str.options[i.VIO.str]
    names(Point.mat.list)[(i.n-1)*length(VIO.str.options)+i.VIO.str] = paste("n=",n,"VIO=",VIO.str,sep="")
    names(CI.mat.list)[(i.n-1)*length(VIO.str.options)+i.VIO.str] = paste("n=",n,"VIO=",VIO.str,sep="")
  }
}

colMeans(Point.mat.list[[1]]); colMeans(Point.mat.list[[2]]); 

rm(list=setdiff(ls(), c("Point.mat.list","CI.mat.list")))
filename = paste("RDatas/CIIV_CI.RData", sep="")
save.image(filename)
