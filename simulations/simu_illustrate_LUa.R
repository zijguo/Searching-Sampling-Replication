...
########### Part-2: Simulations ##########
# matrix storing info
Cov.mat.Sear = Leng.mat.Sear = Point.mat.Sear = Cov.mat.Samp = Leng.mat.Samp = Point.mat.Samp = matrix(NA, nrow=nsim, ncol=7)
for(i.sim in 1:nsim){
  ...
  ## [L, U] = [-10,10]
  beta.grid.TSHT.list[[6]] = grid.CI(matrix(c(-10,10), nrow=1), grid.size=n^(-0.6))
  ## [L, U] = [-20,20]
  beta.grid.TSHT.list[[7]] = grid.CI(matrix(c(-20,20), nrow=1), grid.size=n^(-0.6))
...
rm(list=setdiff(ls(), c("setting", "IV.str", "VIO.str", "n", 
                        "Cov.mat.Sear", "Leng.mat.Sear", "Point.mat.Sear",
                        "Cov.mat.Samp", "Leng.mat.Samp", "Point.mat.Samp", "sim.round")))
filename = paste("Illustrate_LUa-Homo-Setting", setting, "-Strength", IV.str, "-Violation", VIO.str, "-n", n,"-SimRound",sim.round, ".RData", sep="")
save.image(filename)
