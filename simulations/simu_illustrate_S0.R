...
### Author: Zhenyu Wang
### Date: 05/15/2022
### Todo: simulations for the additional setting
library(MASS)
library(intervals)
...
IV.str.weak = 0.075 # {0.05, 0.075, 0.1}
Threshold = 1 # {1,2}
VIO.str = 0.5
IV.str = 0.5
...
if(setting=="S0"){
  ...
  gamma=c(rep(IV.str.weak,2),rep(IV.str,4), IV.str.weak, rep(IV.str, L-7))
}
...
TSHT.VHat.mat = TSHT.SHat.mat = matrix(NA, nrow=nsim, ncol=L)
...
for(i.sim in 1:nsim){
  ...
  TSHT.SHat.mat[i.sim, 1:length(TSHT.temp$SHat)] = TSHT.temp$SHat
  
  # CIIV Method
  CIIV.temp = tryCatch_E(CIIV(Y,D,Z,X,robust=FALSE, firststage=TRUE), ret.obj = list(NULL))
  ...
  if(Threshold==1) TSHT.out <- TSHT.Init_1(ITT_Y, ITT_D, resid_Y, resid_D, WUMat, V.gamma)
  if(Threshold==2) TSHT.out <- TSHT.Init_2(ITT_Y, ITT_D, resid_Y, resid_D, WUMat, V.gamma)
...
rm(list=setdiff(ls(), c("setting","Threshold","IV.str.weak","IV.str", "VIO.str", "n", "Cov.mat", "Leng.mat", "CI.mat", "TSHT.VHat.mat", "TSHT.SHat.mat", "Rule.mat" ,"length.union05", "cover.union05")))
filename = paste("Homo-Setting", setting, "-weak", IV.str.weak,"-Thres",Threshold, "-Strength", IV.str, "-Violation", VIO.str, "-n", n, ".RData", sep="")
save.image(filename)
