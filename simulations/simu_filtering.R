...
colnames(Cov.mat) = colnames(Leng.mat) = c("Filt-YES", "Filt-NO")
CI.mat = matrix(NA, nrow=nsim, ncol=4)
colnames(CI.mat) = c("F-Y-L","F-Y-U","F-N-L","F-N-U")
Rule.mat = matrix(NA, nrow=nsim, ncol=2)
colnames(Rule.mat) = c("Filt-YES", "Filt-NO")
...
for(i.sim in 1:nsim){
  ...
  # TSHT Sampling - not filtering
  out = Searching.CI.sampling(n, ITT_Y, ITT_D, V.Gamma, V.gamma, C, InitiSet=VHat.TSHT,
                              beta.grid = beta.grid.TSHT, filtering=FALSE)
  CI1 = out$CI; rule1 = out$rule
...
rm(list=setdiff(ls(), c("setting", "IV.str", "VIO.str", "n", "Cov.mat", "Leng.mat", "CI.mat", "sim.round")))

filename = paste("Filtering-Homo-Setting", setting, "-Strength", IV.str, "-Violation", VIO.str, "-n", n,"-SimRound",sim.round, ".RData", sep="")
save.image(filename)
